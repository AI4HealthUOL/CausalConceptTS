...
class SSKernelNPLR(nn.Module):
    ...
    def forward(self, L=None, rate=1.0):
        ...
        if L is None:
            L = int(self.L.item() / rate)

        # Increase the internal length if needed
        
        while rate*L > self.L.item():
            self._setup_C(L)
        ...
        if(rate == 1.):
            ...
            omega, z = self._omega(int(discrete_L/rate), dtype=w.dtype, device=w.device, cache=False)
...
class S4(nn.Module):
    ...
    def forward(self, u, **kwargs): # absorbs return_output and transformer src mask
        ...
        # Compute SS Kernel
        k = self.kernel(L=L, rate=kwargs["rate"] if "rate" in kwargs.keys() else 1.0) # (C H L) (B C H L)
